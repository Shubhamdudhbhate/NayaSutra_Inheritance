...
const COURT_ACCESS_CONTROL_ADDR = import.meta.env.VITE_COURT_ACCESS_CONTROL_ADDR || "";
...
export const grantAccess = async (): Promise<void> => {
    if (!window.ethereum) throw new Error("MetaMask not found");
    const provider = new ethers.BrowserProvider(window.ethereum);
    const signer = await provider.getSigner();
    const contract = new ethers.Contract(COURT_ACCESS_CONTROL_ADDR, abi, signer);
