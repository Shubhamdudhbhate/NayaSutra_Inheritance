...
import { EVIDENCE_TYPES } from './evidenceUtils.js';

// --- Configuration ---
// TODO: Replace with your actual Deployed Address
const EVIDENCE_STAGING_ADDR = "0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9";
...
const getEvidenceContract = async (withSigner = false) => {
...
export const generateLawyerPermitSignature = async (caseId, witnessAddress, nonce) => {
    try {
        if (!window.ethereum) throw new Error("MetaMask not found");
        const provider = new ethers.BrowserProvider(window.ethereum);
        const signer = await provider.getSigner();
...
export const getWitnessNonce = async (witnessAddress) => {
...
export const getCaseSubmissions = async (caseId) => {
    try {
        const contract = await getEvidenceContract();
